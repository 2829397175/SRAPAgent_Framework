...
class Genetic_algorithm_Optimizer(BaseOptimizer):
    ...
    def prepare_individual_regressor_with_refinement_experiment(self,
        ...
        pic_path = "EconAgent/optimizer/train_size.png"
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.savefig("EconAgent/optimizer/train_size_all.png")
        plt.clf()
        ...
        plt.ylabel("Loss")
        plt.legend()
        plt.savefig("EconAgent/optimizer/train_size_both.png")
        plt.clf()
        
        assert len(self.used_index) == self.x_used.shape[0] and self.x_used.shape[0] == self.y_used.shape[0]
        self.regressor_model = ridge_model
    ...
    def generate_x(self):
        random_values = {column:self.x_attrs[column].sample().values[0] for column in self.x_attrs.columns}
    ...
    def fit_experiment_evaluate_ver_2(self,
        ...
        import time
        root_optimize = f"EconAgent/optimizer/logs_{self.optimize_type}/{time.time()}"
    ...
    def fit_experiment_evaluate(self,
        ...
        import time
        root_optimize = f"EconAgent/optimizer/logs/{time.time()}"
      
        # self.prepare_individual_regressor()
        
        self.prepare_individual_regressor_with_refinement_experiment(threshold = 0.3,
                                                          sample_size_round = 5,
                                                          max_round = optimize_regressor_rounds,
    ...
    def generate_optimized_result_gt(self,
        ...
        for used_ex_index in used_data.index:
            ex_name = ex_name_map[used_ex_index]
        ...
        try:
            optimized_ex = optimize_log["final_result"]["optimized"]["best_of_all"]["max_pred_y"]
