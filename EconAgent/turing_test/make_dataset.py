...
def read_data(path="EconAgent/LLM_decision_test/data/1.json"):
...
def save_database(communityQA,housetypeQA,houseQA):
    cqa="EconAgent/LLM_decision_test/community_qa.json"
    htqa="EconAgent/LLM_decision_test/housetype_qa.json"
    hqa="EconAgent/LLM_decision_test/house_qa.json"
...
def clear_database():
    filelist=["EconAgent/LLM_decision_test/community_qa.json",
              "EconAgent/LLM_decision_test/housetype_qa.json",
              "EconAgent/LLM_decision_test/house_qa.json"]
...
def batch_read_data():
    ...
    datafile_directory=os.path.join(current_directory,"EconAgent","LLM_decision_test","data")
...
def clear_1_json_memory(json_types):
    ...
    for json_type in json_types:
        json_dir = "EconAgent\LLM_decision_test\old_ver_11_3\qa_translated\judge/finished\{}_qa.json".format(json_type)
        ...
        save_path = "EconAgent\LLM_decision_test\qa_unclear_data/filtered\{}_qa.json".format(json_type)
...
def clear_response(json_types,rate = 0.1):
    ...
    """
    for json_type in json_types:
        json_dir = "EconAgent\LLM_decision_test\qa_unclear_data/filtered\{}_qa.json".format(json_type)
        with open(json_dir,'r',encoding = 'utf-8') as f:
            data = json.load(f)
        ...
        save_path = "EconAgent\LLM_decision_test\qa_clear_data\{}_qa.json".format(json_type)
        save_dir = os.path.dirname(save_path)
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        with open(save_path,'w',encoding = 'utf-8') as f:
...
def concat_translation(json_types=["community","house","housetype"]):
    for json_type in json_types:
        json_dir = "EconAgent\LLM_decision_test\qa_translated\judge/finished copy\{}_qa.json".format(json_type)
        with open(json_dir,'r',encoding = 'utf-8') as f:
            data = json.load(f)
            
        json_ori_dir =  "EconAgent\LLM_decision_test\qa_translated\judge/finished_old_ver\{}_qa.json".format(json_type)
...
def clear_house_json():
    house_json_dir = "EconAgent\LLM_decision_test\qa_translated\judge/finished\house_qa.json"
...
def mix_and_shuffle(save_path = "EconAgent\LLM_decision_test\data\save"
                    ):
    data_paths =["EconAgent/LLM_decision_test/community_qa.json"
    ,"EconAgent/LLM_decision_test/housetype_qa.json"
    ,"EconAgent/LLM_decision_test/house_qa.json"]
