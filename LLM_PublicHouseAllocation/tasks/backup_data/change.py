import json
import pandas as pd
data_dir="LLM_PublicHouseAllocation/tasks/PHA_50tenant_3community_19house/backup_data/community.json"
...
for c_id,c_info in community.items():
    sum_num = 0
    for k,info in c_info.items():
        if isinstance(info,dict):
            sum_num+=len(info.get("index"))
    
    c_info["sum_num"] = sum_num

with open(data_dir, 'w',encoding='utf-8') as f:
    json.dump(community, f, indent=4,ensure_ascii=False)
